# -*- coding: utf-8 -*-
"""
Created on Wed May 11 15:28:06 2022
...
Description: the impact of coefficients of variation
...
df1 = df[(df.serviceRate == 0.6) & (df.holdCost == 0) & (df.price == 16) & (df['scenario number']==3125)]
plt.close('all')
plt.figure()
fig1 = sns.boxplot(x="demand mode", y="sim extend SAA service", hue="sigmaCoe",
...
plt.figure()
fig2 = sns.boxplot(x="demand mode", y="sim SAA service rate", hue="sigmaCoe",
                 data=df1, palette="Set3")
plt.axhline(y=60, xmin=0.01, xmax=0.99, ls='--', label = 'required service rate')
plt.xticks(np.arange(0, 10), demandPatterns)
plt.xlabel('demand patterns')
...
plt.figure()
fig3 = sns.boxplot(x="demand mode", y="sim extend SAA obj", hue="sigmaCoe", data=df1, palette="Set3")
plt.xticks(np.arange(0, 10), demandPatterns)
plt.xlabel('demand patterns')
...
plt.figure()
fig4 = sns.boxplot(x="demand mode", y="sim SAA obj", hue="sigmaCoe",
                 data=df1, palette="Set3")
plt.xticks(np.arange(0, 10), demandPatterns)
plt.xlabel('demand patterns')
plt.ylabel('survival probability')
...
df2 = df1[['demand mode', 'sigmaCoe', 'sim extend SAA service', "sim SAA service rate", "sim extend SAA obj", "sim SAA obj"]].round(1)
service_values1 = df2.groupby(['demand mode', 'sigmaCoe'])["sim extend SAA service"].apply(lambda df1: df1.reset_index(drop=True)).unstack()
service_values2 = df2.groupby(['demand mode', 'sigmaCoe'])["sim SAA service rate"].apply(lambda df1: df1.reset_index(drop=True)).unstack()
service_values3 = df2.groupby(['demand mode', 'sigmaCoe'])["sim extend SAA obj"].apply(lambda df1: df1.reset_index(drop=True)).unstack()
service_values4 = df2.groupby(['demand mode', 'sigmaCoe'])["sim SAA obj"].apply(lambda df1: df1.reset_index(drop=True)).unstack()


names = ['sigmaCoe_extendService','sigmaCoe_SAAService', 'sigmaCoe_extendObj', 'sigmaCoe_SAAObj']
