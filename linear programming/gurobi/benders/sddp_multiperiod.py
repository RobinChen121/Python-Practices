...
I_sub = [[m_sub[t][j].addVar(lb = -GRB.INFINITY, vtype = GRB.CONTINUOUS) for j in range(t_nodeNum[t])] for t in range(T)]
...
while iter < iter_num:   
    ...
    for t in range(T):
        ...
        for j in range(t_nodeNum[t]): 
            ...
            if t == 0:   
                m_sub[t][j].setObjective(vari_cost*q_sub[t][j]+h*max_(I_sub[t][j], 0)+pi*max_(-I_sub[t][j], 0)+theta_sub[t][j], GRB.MINIMIZE)
                m_sub[t][j].addConstr(I_sub[t][j] == q_value - demand)
                m_sub[t][j].write('test.lp')
                print('')               
            else:
                if t == T - 1:                   
                    m_sub[t][j].setObjective(h*max(I_sub[t][j], 0)+pi*max(-I_sub[t][j], 0), GRB.MINIMIZE)
                    m_sub[t][j].write('test.lp')
        ...
        if t == 0:
            m.addConstr(theta >= avg_obj-pi*(q-q_value)) # just the benders optimality cut, same as the above constraint
            m.write('test.lp')
            print(end='')
        else:
            for j in range(t_nodeNum[t-1]):
                index = node_index[t-1][j][0]
                m_sub[t-1][j].addConstr(theta_sub[t-1][j] >= pi*(I_sub[t-1][j] + q_sub[t-1][j]-demand))
                m_sub[t-1][j].update()
