...
def select_mip(scenario_selected, demand_scenarios, demand_possibility, booming_demand, T, delay_length):
    ...
    N = len(demand_scenarios)  # number of items
                ...
                for t in range(T):
                    ...
                    if t == 0:
                        ...
                        m.addConstr(I[t][n][s] >= ini_I[n] + Q[t][n][s] - demand_scenarios[index2][index][n] - (
                                1 - delta[t][n][s]) * MM)
                        m.addConstr(
                            ini_I[n] + Q[t][n][s] - demand_scenarios[index2][index][n] <= delta[t][n][s] * MM - 0.1)
                        m.addConstr(
                            ini_I[n] + Q[t][n][s] >= demand_scenarios[index2][index][n] - (1 - delta[t][n][s]) * MM)
                    else:
                        m.addConstr(
                            I[t][n][s] <= I[t - 1][n][s] + Q[t][n][s] - demand_scenarios[index2][index][n] + (
                                    1 - delta[t][n][s]) * MM)
                        m.addConstr(
                            I[t][n][s] >= I[t - 1][n][s] + Q[t][n][s] - demand_scenarios[index2][index][n] - (
                                    1 - delta[t][n][s]) * MM)
