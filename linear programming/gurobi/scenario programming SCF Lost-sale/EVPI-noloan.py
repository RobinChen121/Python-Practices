...
for s in range(S):
    ...
    print(mean_demand)
    _, scenario_values[s] = determin_noloan.mip(mean_demand, T, booming_demand, ini_cash, overhead_cost, delay_length)
...
Qv, _ = determin_noloan.mip(mean_demands12, T, booming_demand, ini_cash, overhead_cost,
                                   delay_length)  # the solution of the deterministic model
    ...
    for n in range(N):
        for t in range(T):
            ...
            if t == delay_length:
                ...
                Q[t - delay_length][n] = min(Qv[t - delay_length][n], Qbound)
                R[t][n][s] = prices[n] * (
                        I[t - delay_length - 1][n][s] + Q[t - delay_length][n] -
