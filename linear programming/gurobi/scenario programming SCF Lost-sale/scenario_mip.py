...
def mip_fixQ0Credit(Q_range, samples, scenarioLink, scenario_probs, scenario_permulations, booming_demand, B, delay_length, prices, vari_costs, overhead_cost, ini_cash, ini_I, discount_rate, ro):
    S = len(scenario_permulations)
    N = len(samples[0])
    T = len(scenario_permulations[0])
    ...
    #    Q0 = [m.addVar(vtype = GRB.CONTINUOUS) for n in range(N)]
        ...
        for s in range(S):
            for t in range(T):
                ...
                try:
                    if t == 0:
                        C[t][s] = ini_cash + revenue_total[t][s] - vari_costs_total[t][s] - overhead_cost[t] + B
                    else:
                        C[t][s] = C[t-1][s] + revenue_total[t][s] - vari_costs_total[t][s]- overhead_cost[t]
                except:
                    print(n)   
