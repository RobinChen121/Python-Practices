...
first_write = False # whether first output the result to a csv file
stop_condition = 'iter_limit'
Ns = [5, 10, 15, 20, 25]
realization_nums = [5, 10, 15, 20, 25]
iter_limits = [10, 15, 20, 30, 35]
stationarys = [False, True]

grids = [(x, y, z, n) for x in stationarys for y in iter_limits for z in realization_nums for n in Ns]
for stationary, iter_limit, realization_num, N in grids:
    
    opt = 167.31 if stationary == True else 215.48
    ini_I = 0
    ini_cash = 0
    vari_cost = 1
    price = 10
    ...
    unit_hold_cost = 0
    unit_salvage = 0.5
    mean_demands = [10, 20, 10, 20] if stationary == False else [15, 15, 15, 15]
    T = len(mean_demands)
       
    # realization_num = 10    
    sample_nums = [realization_num for t in range(T)]
    overhead_cost = [50 for t in range(T)]
    ...
    while run < 10:
        ...
        content = [ini_cash, ini_I, price, vari_cost, unit_salvage, r0, r1, r2, U, overhead_cost, mean_demands,\
                   stationary, realization_num, N, cut_select_num, iter_limit, time_limit, cpu_time, iter, stop_condition, final_value, Q1, opt, gap]
