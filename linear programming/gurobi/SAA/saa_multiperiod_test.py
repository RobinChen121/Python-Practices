...
try:
    ...
    cash = [[LinExpr() for s in range(scenario_num)] for t in range(T)]
    ...
    for t in range(T):
        for s in range(scenario_num):
            if t == 0 and T > 1: # end-of-period cash balance in the first period
                cash[t][s] = ini_cash + price * Rd[t][s] - vari_cost * Q[t][0]
            elif t == 0 and T <= 1:
                cash[t][s] = ini_cash + price * Rd[t][s] - vari_cost * Q[t][0] + sal_value * Iplus[t][s]
            elif t == T - 1 and T > 1:
                cash[t][s] = cash[t-1][s] + price * Rd[t][s]  - vari_cost * Q[t][s] + sal_value * Iplus[t][s]
            else:
                cash[t][s] = cash[t-1][s] + price * Rd[t][s]  - vari_cost * Q[t][s]
        
    
    total_expect_cash = sum(cash[T - 1]) / scenario_num
    m.update()
    m.setObjective(total_expect_cash, GRB.MAXIMIZE)
    ...
    for t in range(T):
        for s in range(scenario_num):
            if t == 0:
                ...
                m.addConstr(vari_cost * Q[t][s] <= cash[t-1][s])
    ...
    print('final expected cash is:  %.2f ' % total_expect_cash.getValue())
