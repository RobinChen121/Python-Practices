...
# for determin situation, each ref scenario take mean values
def mip_determin(ref_s, B, delay_length, prices, vari_costs, overhead_cost, ini_cash, ini_I, discount_rate, r0):
    N = len(ref_s[0])
    T = len(ref_s)
...
def mip_pairS(ref_range, samples, scenario_permulations, B, delay_length, prices, vari_costs, overhead_cost, ini_cash, ini_I, discount_rate, ro):
    ...
    #    Q0 = [m.addVar(vtype = GRB.CONTINUOUS) for n in range(N)]
        ...
        final_cash = (C[T-1][0] + discounted_cash[0])/ SS + (C[T-1][1] + discounted_cash[0])*(SS-1)/ SS
