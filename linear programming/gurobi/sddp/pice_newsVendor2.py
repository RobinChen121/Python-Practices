...
# -*- coding: utf-8 -*-
"""
Created on Sat Apr  1 11:03:26 2023
...
while iter <= iter_num:   
    ...
    pi_rhs_values = [[0 for s in range(t_nodeNum[t])] for t in range(T)] 
    obj_remant = [[0 for s in range(t_nodeNum[t])] for t in range(T)]
    ...
    for t in range(T):       
        for j in range(t_nodeNum[t]): 
            ...
            if t == 0:   
                if T > 1:
                    m_sub[t][j].setObjective(vari_cost*q_sub[t][j] + unit_hold_cost*I_sub[t][j] - price*(ini_I + q_value - I_sub[t][j]) +theta_sub[t][j], GRB.MINIMIZE)
                    m_sub[t][j].addConstr(theta_sub[t][j] >= theta_iniValue*(T-1-t))
                    obj_remant[t][j] = - price*(ini_I + q_value)
                else:
                    last_index = 0
                    for k in node_index[t - 1]:
                        if node_index[t][j][0] in k:
                            last_index = node_index[t - 1].index(k)
                    m_sub[t][j].setObjective(unit_hold_cost*I_sub[t][j] - price*(I_sub_values[t-1][last_index] + q_sub_values[t-1][last_index] - I_sub[t][j]), GRB.MINIMIZE)
                    obj_remant[t][j] = - price*(I_sub_values[t-1][last_index] + q_detail_values[iter-1][t-1][last_index])
                ...
                if t == T - 1:                              
                    ...
                    m_sub[t][j].setObjective(vari_cost*q_sub[t][j] + unit_hold_cost*I_sub[t][j] + price*(I_sub_values[t-1][last_index] + q_detail_values[iter-1][t-1][last_index] - I_sub[t][j]) +theta_sub[t][j], GRB.MINIMIZE)
                    m_sub[t][j].addConstr(theta_sub[t][j] >= theta_iniValue*(T-1-t))
                
                obj_remant[t][j] = - price*(I_sub_values[t-1][last_index] + q_sub_values[t-1][last_index])
        ...
        if t == 0:
            # should have more
            m.addConstr(theta >= avg_pi*q + avg_pi_rhs - price*q) # just the benders optimality cut, same as the above constraint
            # m.write('test.lp')
            print(end='')
        else:
            for j in range(t_nodeNum[t-1]):             
                m_sub[t-1][j].addConstr(theta_sub[t-1][j] >= avg_pi*(I_sub[t-1][j] + q_sub[t-1][j]) + avg_pi_rhs - price*(I_sub[t-1][j] + q_sub[t-1][j]))
                m_sub[t-1][j].update()
                print(end='')
