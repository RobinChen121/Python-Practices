...
slope1_stage = []
intercept1_stage = []
slopes1 = [[ [] for n in range(N)] for t in range(T-1)]
...
while iter < iter_num:  
    ...
    pi_values1 = [[[0  for k in range(sample_nums[t])] for n in range(N)] for t in range(T)]
    ...
    for t in range(T - 1, -1, -1):
       for n in range(N):
            ...
            for k in range(K):
                ...
                if t < T - 1:
                    num_con = len(pi)
                    pi_expect = sum(pi_values1[t+1][n])/K
