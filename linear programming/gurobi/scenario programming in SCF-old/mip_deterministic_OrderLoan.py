...
try:
    ...
    g = [[m.addVar(vtype = GRB.CONTINUOUS) for t in range(T)] for n in range(N)] # order-loan used 
    
    R = [[LinExpr() for t in range(T + delay_length)] for n in range(N)] # LinExpr, revenue for each product in each period
    I = [[LinExpr() for t in range(T)] for n in range(N)] # end-of-period inventory in each period for each product
    ...
    for n in range(N):
        for t in range(T + delay_length):
            if t < delay_length:
                ...
                R[n][t] = prices[n] * (g[n][t]+mean_demands[n] - w[n][t-delay_length]-g[n][t-delay_length]-g[n][t-delay_length]*(1+ro)**delay_length)
            else:
                R[n][t] = prices[n] * (mean_demands[n] - w[n][t-delay_length]-g[n][t-delay_length]-g[n][t-delay_length]*(1+ro)**delay_length)
                
    # objective function          
    discounted_cash = LinExpr()
    for n in range(N):
        for k in range(delay_length):
            discounted_cash = discounted_cash + R[n][T+k] / (1 + discount_rate)**(k+1)    
    ...
    for n in range(N):
        for t in range(T):
            m.addConstr(g[n][t] <= mean_demands[n] - w[n][t])
