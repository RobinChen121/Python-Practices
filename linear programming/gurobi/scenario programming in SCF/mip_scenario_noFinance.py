...
def lp():    
    ...
    try:
        ...
        C = [[m.addVar(vtype = GRB.CONTINUOUS) for s in range(S)] for t in range(T)] # LinExpr, end-of-period cash in each period in each scenario
        R = [[[m.addVar(vtype = GRB.CONTINUOUS)for s in range(S)] for n in range(N)] for t in range(T)] # LinExpr, revenue for each product in each period in each scenario
        ...
        for s in range(S):
            for n in range(N): 
                for t in range(T):                         
                    ...
                    m.addConstr(R[t][n][s] == prices[n] * (demand_scenarios[index][n] - w[t][n][s]))
        ...
        for s in range(S):
            for t in range(T):
                ...
                if t == 0:
                    if t == delay_length:
                        m.addConstr(C[t][s] == ini_cash + revenue_total[s][t] - vari_costs_total[s][t] - overhead_cost)
                    else:
                        m.addConstr(C[t][s] == ini_cash - vari_costs_total[s][t] - overhead_cost)
                else:
                    if t < delay_length:
                        m.addConstr(C[t][s] == C[t-1][s] - vari_costs_total[s][t] - overhead_cost)
                    else:
                        m.addConstr(C[t][s] == C[t-1][s] + revenue_total[s][t-delay_length] - vari_costs_total[s][t] - overhead_cost)
        for t in range(T):
        ...
        for t in range(T):
         ...
         # solve
        m.update()
        #m.Params.Presolve = 1 # can be -1, 0, 1, 2. More aggressive application of presolve takes more time, but can sometimes lead to a significantly tighter model.
        #m.params.Method = 3 # barrier, simplex, dual simplex to save memory
        m.optimize()
