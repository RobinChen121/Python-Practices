...
iter = 1
iter_num = 10
pi_sub_detail_values = [
    [[[] for s in range(t_nodeNum[t])] for t in range(T)] for iter in range(iter_num)
]
...
while iter <= iter_num:
    ...
    for t in range(T):
        ...
        for j in range(t_nodeNum[t]):
            ...
            if t == 0:
                m_sub[t][j].setObjective(
                    vari_cost * q_sub[t][j]
                    + unit_hold_cost * I_sub[t][j]
                    + unit_back_cost * B_sub[t][j]
                    + theta_sub[t][j],
                    GRB.MINIMIZE,
                )
                m_sub[t][j].addConstr(
                    I_sub[t][j] - B_sub[t][j] == ini_I + q_value - demand
                )
                print("")
            else:
                if t == T - 1:
                    m_sub[t][j].setObjective(
                        unit_hold_cost * I_sub[t][j] + unit_back_cost * B_sub[t][j],
                        GRB.MINIMIZE,
                    )
                else:
                    m_sub[t][j].setObjective(
                ...
                m_sub[t][j].addConstr(
                    ...
                    - B_sub_values[t - 1][last_index]
                    + q_detail_values[iter - 1][t][last_index]
                    - demand
                )
                print(end="")
        ...
        # this is not the formal handling of ND, nor the formal handling of SDDP
        ...
        if t == 0:
            m.addConstr(
                theta >= avg_pi * q + avg_pi_rhs
            )  # just the benders optimality cut, same as the below constraint
            # m.write('test.lp')

        else:
            for j in range(t_nodeNum[t - 1]):
                m_sub[t - 1][j].addConstr(
                    theta_sub[t - 1][j]
                    >= avg_pi * (I_sub[t - 1][j] - B_sub[t - 1][j] + q_sub[t - 1][j])
                    + avg_pi_rhs
                )
                m_sub[t - 1][j].update()
                # m_sub[t][j].write('iter' + str(iter) + '_sub_' + str(t+1) + '^' + str(j+1) + '.lp')
