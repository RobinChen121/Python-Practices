...
@disp:  ND for newsvendor lead time
...
while iter < iter_num:
    ...
    q_forward = [
        [
            m_sub[t][n].addVar(
                vtype=GRB.CONTINUOUS, name="q_" + str(t + 2) + "^" + str(n + 1)
            )
            for n in range(N)
        ]
        for t in range(T - 1)
    ...
    q_pre = [
        [
            m_sub[t][n].addVar(
                vtype=GRB.CONTINUOUS, name="qpre_" + str(t + 1) + "^" + str(n + 1)
            )
            for n in range(N)
        ]
        for t in range(T - 1)
    ...
    I = [
        [
            m_sub[t][n].addVar(
                vtype=GRB.CONTINUOUS, name="I_" + str(t + 1) + "^" + str(n + 1)
            )
            for n in range(N)
        ]
        for t in range(T)
    ...
    B = [
        [
            m_sub[t][n].addVar(
                vtype=GRB.CONTINUOUS, name="B_" + str(t + 1) + "^" + str(n + 1)
            )
            for n in range(N)
        ]
        for t in range(T)
        ...
        for n in range(N):
            ...
            if t == T - 1:
                m_sub[t][n].setObjective(
                    unit_hold_cost * I[t][n] + unit_back_cost * B[t][n], GRB.MINIMIZE
                )
            else:
                m_sub[t][n].setObjective(
                    vari_cost * q_forward[t][n]
                    + unit_hold_cost * I[t][n]
                    + unit_back_cost * B[t][n]
                    + theta_forward[t][n],
                    GRB.MINIMIZE,
                )
            if t == 0:
            ...
            if t < T - 1:
                for i in range(iter):
                    m_sub[t][n].addConstr(
                        theta_forward[t][n]
                        >= slopes1[i][t][n] * q_forward[t][n]
                        + slopes2[i][t][n] * (I[t][n] - B[t][n] + q_pre[t][n])
                        + intercepts[i][t][n]
                    )
            ...
            if iter == 2 and t == 0:
                m_sub[t][n].write(
                    "iter" + str(iter) + "_sub_" + str(t) + "^" + str(n) + ".lp"
                )
                m_sub[t][n].write(
                    "iter" + str(iter) + "_sub_" + str(t) + "^" + str(n) + ".sol"
                )
                pass
    ...
    for t in range(T - 1, -1, -1):
        for indice in node_index[t]:
            ...
            for n in indice:
                ...
                for kk in range(
                    2, num_con
                ):  # actually iter > 0 and t< T-1, below for cuts in iter > 1
