...
slopes1 = [
    [[0 for n in range(t_nodeNum[t])] for t in range(T)] for iter in range(iter_num)
]
slopes2 = [
    [[0 for n in range(t_nodeNum[t])] for t in range(T)] for iter in range(iter_num)
]
...
while iter < iter_num:
    ...
    m.setObjective(
        overhead_cost[0] + vari_cost * q + r1 * W1 - r0 * W0 + theta, GRB.MINIMIZE
    )
    ...
    q_sub = [
        [
            m_sub[t][j].addVar(
                vtype=GRB.CONTINUOUS, name="q_" + str(t + 2) + "^" + str(j + 1)
            )
    ...
    I_sub = [
        [
            m_sub[t][j].addVar(
                vtype=GRB.CONTINUOUS, name="I_" + str(t + 1) + "^" + str(j + 1)
            )
            for j in range(t_nodeNum[t])
        ]
        for t in range(T)
    ...
    B_sub = [
        [
            m_sub[t][j].addVar(
                vtype=GRB.CONTINUOUS, name="B_" + str(t + 1) + "^" + str(j + 1)
            )
            for j in range(t_nodeNum[t])
        ]
        for t in range(T)
    ...
    C_sub = [
        [
            m_sub[t][j].addVar(
                lb=-GRB.INFINITY, name="C_" + str(t + 1) + "^" + str(j + 1)
            )
            for j in range(t_nodeNum[t])
        ]
        for t in range(T)
    ...
    W0_sub = [
        [
            m_sub[t][j].addVar(
                vtype=GRB.CONTINUOUS, name="W0_" + str(t + 1) + "^" + str(j + 1)
            )
            for j in range(t_nodeNum[t])
        ]
        for t in range(T - 1)
    ...
    W1_sub = [
        [
            m_sub[t][j].addVar(
                vtype=GRB.CONTINUOUS, name="W1_" + str(t + 1) + "^" + str(j + 1)
            )
            for j in range(t_nodeNum[t])
        ]
        for t in range(T - 1)
    ...
    for t in range(T):
        for n in range(t_nodeNum[t]):
            ...
            if t < T - 1:
                for k in range(iter):
                    if t == 0:
                        m_sub[t][n].addConstr(
                            theta_sub[t][n]
                            >= slope1[k][t] * (I_sub[t][n] + q_sub[t][n])
                            + slope2[k][t]
                            * (
                                -vari_cost * q_sub[t][n]
                                - r1 * W1_sub[t][n]
            ...
            if t == T - 1:
                m_sub[t][n].setObjective(
                    -price * (demand - B_sub[t][n]) - unit_sal * I_sub[t][n],
                    GRB.MINIMIZE,
                )
            else:
                m_sub[t][n].setObjective(
                    -price * (demand - B_sub[t][n])
                    + overhead_cost[t + 1]
                    + vari_cost * q_sub[t][n]
                    + r1 * W1_sub[t][n]
                    - r0 * W0_sub[t][n]
            ...
            if t < T - 1:
                ...
                m_sub[t][n].addConstr(
                    C_sub[t][n] - vari_cost * q_sub[t][n] - W0_sub[t][n] + W1_sub[t][n]
                    == overhead_cost[t]
                )

            if t == 0:
                m_sub[t][n].addConstr(
                    I_sub[t][n] - B_sub[t][n] == ini_I + q_value - demand
                )
            else:
                m_sub[t][n].addConstr(
                    I_sub[t][n] - B_sub[t][n]
                    == I_sub_values[t - 1][last_index]
                    + q_detail_values[iter][t][last_index]
                    - demand
                )
            if t == 0:
                m_sub[t][n].addConstr(
                    C_sub[t][n] + price * B_sub[t][n]
                    == ini_cash
                    - overhead_cost[t]
                    - vari_cost * q_value
                    - r1 * W1_detail_values[iter][0]
                    + r0 * W0_detail_values[iter][0]
                    + price * demand
                )
            else:
                m_sub[t][n].addConstr(
                    C_sub[t][n] + price * B_sub[t][n]
                    == C_sub_values[t - 1][last_index]
                    - overhead_cost[t]
                    - vari_cost * q_detail_values[iter][t][last_index]
                    - r1 * W1_detail_values[iter][t][last_index]
                    + r0 * W0_detail_values[iter][t][last_index]
                    + price * demand
                )
            ...
            pi_rhs += (
                -pi[-2] * demand
                - pi[-1] * overhead_cost[t]
                + pi[-1] * price * demand
                - price * demand
            )  # put here is better because of demand
            if t < T - 1:
        ...
        if t == 0:
            ...
            m.addConstr(
                theta
                >= avg_pi1 * q
                + avg_pi2 * (-vari_cost * q - r1 * W1 + r0 * W0)
                + avg_pi_rhs
            )
            m.update()
        else:
