...
datas = pd.read_excel(r'\\Mac\Home\Documents\gitOld\Numerical-tests\joint chance datas\2022-08-03\rolling-tests.xls')
...
df1 = df1[df1['sample one period'] == 9]
...
header_txt = 'demand-mode rolling-obj1 rolling-service2 rolling-obj2 rolling-service2 rolling-obj3 rolling-service3'
output = rolling_values1.merge(rolling_values2, on = 'demand mode')
output = output.merge(rolling_values3, on = 'demand mode')
np.savetxt(r'\\Mac\Home\Documents\gitOld\Numerical-tests\joint chance datas\2022-08-03\rolling_output1.txt', output.values, fmt='%.1f', header = header_txt, comments='', delimiter=' ')
