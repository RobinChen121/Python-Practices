# -*- coding: utf-8 -*-
"""
Created on Mon May  2 13:43:44 2022
...
Description: about the impact of requried service rate
...
plt.close('all')
plt.figure()
fig1 = sns.boxplot(x="demand mode", y="sim extend SAA service", hue="serviceRate",
                 data=df1, palette="Set3")
plt.xticks(np.arange(0, 10), demandPatterns)
plt.xlabel('demand patterns')
plt.ylabel('service rate')
plt.ylim([0, 100])
plt.axhline(y=50, xmin=0.01, xmax=0.99, ls='--', label = 'required service rate')
plt.axhline(y=60, xmin=0.01, xmax=0.99, ls='--', label = 'required service rate')
plt.axhline(y=70, xmin=0.01, xmax=0.99, ls='--', label = 'required service rate')
plt.legend()

plt.figure()
fig2 = sns.boxplot(x="demand mode", y="sim SAA service rate", hue="serviceRate",
                 data=df1, palette="Set3")
plt.axhline(y=50, xmin=0.01, xmax=0.99, ls='--', label = 'required service rate')
plt.axhline(y=60, xmin=0.01, xmax=0.99, ls='--', label = 'required service rate')
plt.axhline(y=70, xmin=0.01, xmax=0.99, ls='--', label = 'required service rate')
...
plt.figure()
fig3 = sns.boxplot(x="demand mode", y="sim extend SAA obj", hue="serviceRate", data=df1, palette="Set3")
plt.xticks(np.arange(0, 10), demandPatterns)
plt.xlabel('demand patterns')
...
plt.figure()
fig4 = sns.boxplot(x="demand mode", y="sim SAA obj", hue="serviceRate",
                 data=df1, palette="Set3")
plt.xticks(np.arange(0, 10), demandPatterns)
plt.xlabel('demand patterns')
plt.ylabel('survival probability')
...
df2 = df1[['demand mode', 'serviceRate', 'sim extend SAA service', "sim SAA service rate", "sim extend SAA obj", "sim SAA obj"]].round(1)
service_values1 = df2.groupby(['demand mode', 'serviceRate'])["sim extend SAA service"].apply(lambda df1: df1.reset_index(drop=True)).unstack()
service_values2 = df2.groupby(['demand mode', 'serviceRate'])["sim SAA service rate"].apply(lambda df1: df1.reset_index(drop=True)).unstack()
service_values3 = df2.groupby(['demand mode', 'serviceRate'])["sim extend SAA obj"].apply(lambda df1: df1.reset_index(drop=True)).unstack()
service_values4 = df2.groupby(['demand mode', 'serviceRate'])["sim SAA obj"].apply(lambda df1: df1.reset_index(drop=True)).unstack()





names = ['service_extendService','service_SAAService', 'service_extendObj', 'service_SAAObj']
